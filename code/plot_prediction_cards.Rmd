...
plot_state_forecasters_ <- function(df) {
  plot_state_forecasters(df %>% filter(geo_value != "us"), exclude_geos = exclude_geos, start_day = "2022-01-01", ncol = 2, offline_signal_dir = here::here("cache/signals")) %>% suppressMessages() %>% suppressWarnings()
}

plot_nation_forecasters_ <- function(df) {
  plot_nation_forecasters(df %>% filter(geo_value == "us"), exclude_geos = exclude_geos, start_day = "2022-01-01", offline_signal_dir = here::here("cache/signals")) %>% suppressMessages() %>% suppressWarnings()
}
