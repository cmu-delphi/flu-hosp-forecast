...
plot_state_forecasters(df %>% filter(geo_value != "us"), exclude_geos = exclude_geos, start_day = "2022-01-01", ncol = 2) %>%
  suppressMessages() %>%
  suppressWarnings() %>%
  plotly::ggplotly()
```
...
# setup the plot and join corrections to the truth
plot_nation_forecasters(df %>% filter(geo_value == "us"), exclude_geos = exclude_geos, start_day = "2022-01-01") %>%
  suppressMessages() %>%
  suppressWarnings() %>%
  plotly::ggplotly()
```
